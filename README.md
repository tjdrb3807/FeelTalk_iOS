...
* 필로우독은 소셜 로그인 기반 인증 구조를 사용하여 로그인과 회원가입을 하나의 플로우로 통합했습니다. 인증 성공 시 사용자 계정을 생성·로그인하는 방식으로 동작하여, 초기 진입 과정을 단순화했습니다.
* 성인 대상 서비스 특성상 휴대폰 본인 인증을 통한 성인 인증을 필수 단계로 포함하고 있으며,
인증 상태에 따라 화면 전환 및 다음 단계 접근을 명확히 제어합니다.
* 본인 인증 이후에는 닉네임을 설정하고, 커플 서비스 특성을 반영한 코드 기반 사용자 연결 구조를 제공합니다.

### 시그널 전송
![시그널 전송 플로우](./image/FeelTalk_Signal_Flow.png)
* 시그널 전송은 사용자의 현재 기분 상태를 상대방에게 전달하고,
서로의 상태를 실시간으로 확인할 수 있도록 설계된 메인 서비스입니다.
* 사용자는 홈 화면에서 상대방의 시그널을 확인하고,
시그널 변경 화면을 통해 자신의 기분 상태를 선택할 수 있습니다.
선택된 시그널은 서버에 저장되며, 커플로 연결된 상대방에게 전달됩니다.

### 오늘의 질문
![오늘의 질문 플로우](./image/FeelTalk_Question_Flow.png)
* 오늘의 질문은 커플 간 대화를 자연스럽게 유도하기 위해
매일 새로운 질문을 랜덤하게 제공하는 커뮤니케이션 기능입니다.

* 사용자는 매일 제공되는 질문에 대해 자신의 답변을 작성할 수 있으며,
답변은 커플 관계로 연결된 상대방과 공유됩니다.
이를 통해 일상적인 대화뿐 아니라, 평소 쉽게 꺼내기 어려운 주제에도
부담 없이 접근할 수 있도록 설계했습니다.

* 상대방이 아직 답변하지 않은 경우,
‘꼭 찌르기’ 기능을 통해 질문 응답을 부드럽게 요청할 수 있습니다.
이 기능은 강한 알림이나 강제성을 지양하고,
가벼운 상호작용을 통해 대화 참여를 유도하는 것을 목표로 합니다.

### 첼린지
![챌린지 플로우](./image/FeelTalk_Challenge_Flow.png)
* 챌린지 등록은 커플이 함께 목표를 설정하고,
일정 기간 동안 이를 공유·실천할 수 있도록 지원하는 참여형 서비스입니다.

* 사용자는 직접 챌린지를 생성하여 제목, 내용, 마감일을 설정할 수 있으며,
생성된 챌린지는 커플 관계로 연결된 상대방과 공유됩니다.
챌린지는 진행 중 / 완료 상태로 구분되어 관리되며,
각 상태에 따라 화면에 노출되는 목록이 달라집니다.

* 진행 중인 챌린지는 남은 기간(D-day)을 기준으로 표시되며,
완료된 챌린지는 히스토리 형태로 확인할 수 있습니다.
이를 통해 단발성 이벤트가 아닌, 커플 간의 공동 경험 기록으로 기능하도록 설계했습니다.

### 화면잠금
![화면잠금 플로우](./image/FeelTalk_LockScreen_Flow.png)
* 화면 잠금은 커플 서비스 특성상 발생할 수 있는 민감 정보 노출을 방지하기 위해 제공되는 보안 기능입니다.

* 사용자는 설정 화면에서 화면 잠금을 활성화할 수 있으며,
잠금 활성화 시 앱 진입 또는 포그라운드 전환 시 사용자 설정 비밀번호 입력을 요구합니다.
이를 통해 타인이 앱 화면을 우연히 확인하는 상황에서도 개인정보를 보호할 수 있도록 설계했습니다.

* 비밀번호 분실 상황을 고려하여 보안 질문 기반 비밀번호 재설정 기능을 함께 제공하며,
사용자가 설정한 질문과 답변을 통해 비밀번호를 안전하게 복구할 수 있습니다.
