...
final class CustomTextField: UITextField {
    ...
    private func setProperties() {
        font = UIFont(name: CommonFontNameSpace.pretendardRegular,
                      size: CustomTextFieldNameSpace.textSize)
        textColor = .black
        backgroundColor = UIColor(named: CommonColorNameSpace.gray200)
        
        layer.borderColor = UIColor.clear.cgColor
        layer.borderWidth = CustomTextFieldNameSpace.borderWidth
        layer.cornerRadius = CustomTextFieldNameSpace.cornerRadius
        
        clipsToBounds = true
        tintColor = UIColor(named: CommonColorNameSpace.main500)
        
        let leftPaddingView = UIView(frame: CGRect(origin: .zero,
                                                   size: CGSize(width: CustomTextFieldNameSpace.leftPaddingViewWidth,
                                                                height: frame.height)))
